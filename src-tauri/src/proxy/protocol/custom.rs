...
/// Custom (OpenAI-compatible) protocol adapter.
///
/// The user provides the full versioned base_url (e.g. `https://api.deepseek.com/v1`),
/// so we only append `chat/completions` or `models` — no `/v1` prefix.
/// We do NOT use `join_url` here because it strips `/v1` which the user explicitly provided.
